import { type ConfigEnv, type ResolvedConfig, normalizePath, loadEnv } from 'vite'
...
function checkEnvType(env: any) {
  ...
  if (
    ...
    (myEnv.VITE_KINTONE_PLATFORM !== 'APP' && myEnv.VITE_KINTONE_PLATFORM !== 'PORTAL') ||
    (myEnv.VITE_KINTONE_TYPE !== 'DESKTOP' && myEnv.VITE_KINTONE_TYPE !== 'MOBILE')
  ) {
    return false
  }

  // 检查可选的环境变量
  if (myEnv.VITE_KINTONE_APP !== undefined && typeof myEnv.VITE_KINTONE_APP !== 'string') {
    return false
  }
...
export function validateEnv(envConfig: ConfigEnv, viteConfig: ResolvedConfig) {
  ...
  const envDir = viteConfig.envDir ? normalizePath(path.resolve(resolvedRoot, viteConfig.envDir)) : resolvedRoot
...
export async function checkEnv(envConfig: ConfigEnv, viteConfig: ResolvedConfig) {
